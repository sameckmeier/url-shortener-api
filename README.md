...
- **Docker:** I believe using docker in development is a great way to get new engineers up and running on projects without needing to worry about setting up their local environment. It also means that a team has a consistent strategy for running the application in different contexts, such as locally or CI/CD.
...
- **Shortend Url Algorithm:** I wanted to avoid any collision-related issues with generating random, alphanumeric sequences, so I implemented an algorithm that starts with, a. Everytime this service creates a Url record, we increment the sequence, ie, a increments to b, A increments to B, and 0 increments to 1. Once we exhaust all combinations for the current sequence, we append a char to our sequence and reset it.
- **Interactors:** I prefer to store all business logic in service objects since they decouples your business logic, models, and controllers from each other. As the Rails saying goes, "Skinny models and skinny controllers."
